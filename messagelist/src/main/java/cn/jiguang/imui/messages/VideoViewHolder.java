...
import cn.jiguang.imui.utils.TimeFormat;
...
public class VideoViewHolder<Message extends IMessage> extends BaseMessageViewHolder<Message>
    ...
    public void onBind(final Message message) {
        ...
        long nowDate = message.getTimeMillis();
        if (nowDate != 0) {
            if (mPosition < mData.size() && mPosition + 1 < mData.size()) {
                MsgListAdapter.Wrapper wrapper = mData.get(mPosition + 1);
                if (wrapper.getItem() instanceof IMessage) {
                    long lastDate = ((IMessage) wrapper.getItem()).getTimeMillis();
                    if (nowDate - lastDate > 300000) {
                        mDateTv.setText(new TimeFormat(mContext, nowDate).getDetailTime());
                        mDateTv.setVisibility(View.VISIBLE);
                    } else {
                        mDateTv.setVisibility(View.GONE);
                    }
